        ...
        public async Task<int> FetchPass(SocketCommandContext p_Context = null)
        {
            if (m_PlayerID != null)
            {
                ...
                try
                {
                    for (l_Y = 0; l_Y <= l_BiggerLevelID; l_Y++)
                    {
                                                            ...
                                                            if (l_CachedPassedSong.difficulties != null && string.Equals(l_CachedPassedSong.hash, l_Song.hash, StringComparison.CurrentCultureIgnoreCase))
                                                            {
                                                                ...
                                                                if (!l_DiffExist)
                                                                {
                                                                    ...
                                                                    if (!l_OldDiffExist)
                                                                    {
                                                                        ...
                                                                        if (l_Messages[l_MessagesIndex].Length > 2000 - $"<:clap:868195856560582707> Passed ***``{l_Difficulty.name.Replace("`",@"\`").Replace("*",@"\*")} {l_Difficulty.characteristic} - {l_Score.songName}``*** in Level **{l_Y - 1}**\n".Length)
                                                                        {
                                                                            l_MessagesIndex++;
                                                                        }
                                                                        ...
                                                                        l_Messages[l_MessagesIndex] += $"<:clap:868195856560582707> Passed ***``{l_Difficulty.name.Replace("`",@"\`").Replace("*",@"\*")} {l_Difficulty.characteristic} - {l_Score.songName}``*** in Level **{l_Y - 1}**\n";
                                                                        l_Passes++;
                                                                        l_PassesPerLevel++;
                        ...
                        if (l_NumberOfDifficulties > 0)
                        {
                            switch (l_PassesPerLevel * 100 / l_NumberOfDifficulties)
                            {
                                case 0:
                                {
                                    SetGrindInfo(l_Y + 1, new List<bool> { false }, -1, null, -1);
                                    break;
                                }
                                case <= 39:
                                {
                                    l_Plastic = 1;
                                    SetGrindInfo(l_Y + 1, new List<bool> { true }, -1, null, -1);
                                    break;
                                }
                                case <= 69:
                                {
                    ...
                    SetGrindInfo(-1, null, l_TotalAmountOfPass, null, l_Points);
                    ReWritePass();
