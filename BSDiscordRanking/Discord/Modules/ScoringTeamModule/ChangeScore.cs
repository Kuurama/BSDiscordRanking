...
namespace BSDiscordRanking.Discord.Modules.ScoringTeamModule
{
    ...
    public partial class ScoringTeamModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task EditScore(string p_DiscordOrScoreSaberID = null, string p_LeaderboardID = null, int p_Score = int.MaxValue)
        {
            if (string.IsNullOrEmpty(p_DiscordOrScoreSaberID) || string.IsNullOrEmpty(p_LeaderboardID) || p_Score == int.MaxValue)
            {
                await ReplyAsync($"> :x: Seems like you didn't used the command correctly, use: `{BotHandler.m_Prefix}editscore [DiscordOrScoreSaberID] [LeaderboardID] [Score]`");
                return;
            }
            ...
            if (int.TryParse(p_LeaderboardID, out int l_LeaderboardID))
            {
                ...
                if (ulong.TryParse(p_DiscordOrScoreSaberID, out ulong l_ScoreSaberID))
                {
                    ...
                    if (l_Player.m_PlayerScoreCollection != null)
                    {
                        ...
                        if (l_ScoreIndex >= 0)
                        {
                            l_EmbedBuilder.AddField($"{l_Player.m_PlayerFull.name}'s score", $"(#{l_Player.m_PlayerScoreCollection.playerScores[l_ScoreIndex].score.rank}) - {l_Player.m_PlayerScoreCollection.playerScores[l_ScoreIndex].score.baseScore} => (#{-1}) - {p_Score}.");
                            l_Player.m_PlayerScoreCollection.playerScores[l_ScoreIndex].score.baseScore = p_Score;
                            l_Player.m_PlayerScoreCollection.playerScores[l_ScoreIndex].score.rank = -1;
                            l_ScoreChanged = true;
